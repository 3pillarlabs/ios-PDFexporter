...
### We need to copy the Modules folder to the universal directory ###
SIMULATOR_MODULES_DIR="${SIMULATOR_FRAMEWORK_PATH}/Modules/"
UNIVERSAL_MODULES_DIR="${UNIVERSAL_FRAMEWORK_PATH}/Modules/"
if [ -d "${SIMULATOR_MODULES_DIR}" ]; then
cp -r "${SIMULATOR_MODULES_DIR}" "${UNIVERSAL_MODULES_DIR}"
fi
fi
